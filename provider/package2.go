...
func HttpExchange(r *http.Request) (*http.Response, *core.Status) {
	...
	case searchResource:
		return search2[core.Log](r)
	default:
		status := core.NewStatusError(http.StatusNotFound, errors.New(fmt.Sprintf("error invalid URI, resource not found: [%v]", path)))
		return httpx.NewErrorResponse(status), status //Response{StatusCode: http.StatusNotFound}, core.NewStatusError(http.StatusNotFound, errors.New(fmt.Sprintf("error invalid URI, resource not found: [%v]", path)))
	}
}
